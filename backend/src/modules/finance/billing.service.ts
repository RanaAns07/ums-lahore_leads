    ...
    async generateSemesterInvoice(enrollmentId: string): Promise<Invoice> {
        ...
        if (feeStructures.length === 0) {
            throw new NotFoundException(
                `No fee structures found for program ${enrollment.program_id} in semester ${enrollment.semester_id} `,
            );
        }
        ...
        return this.prisma.$transaction(async (tx) => {
            ...
            if (!completeInvoice) {
                throw new Error('Failed to create complete invoice');
            }

            console.log(
                `💰 Invoice generated: ${invoice.id} for ${completeInvoice.enrollment.student_profile.student_id_number} - $${totalAmount}`,
            );
