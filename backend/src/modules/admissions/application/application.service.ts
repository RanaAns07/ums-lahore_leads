...
export class ApplicationService {
     ...
     * Validates minimum required documents (delegated to updateStatus)
     */
    async accept(id: string): Promise<Application> {
        ...
        try {
            ...
            const email = applicant?.person?.user?.email || applicant?.inquiry?.email;
            ...
            if (applicant && email) {
                ...
                console.log(
                    `📧 Acceptance email sent to: ${email}`,
                );
            }
        } catch (emailError) {
            console.error(
